 ...
 * Utility functions for posture data processing and analysis
 */
...
async function fetchAllEntries() {
    try {
        ...
        console.error('[postureHelper] fetchAllEntries error:', err.message);
        return [];
    }
}
...
async function fetchStats() {
    try {
        ...
        console.error('[postureHelper] fetchStats error:', err.message);
        return null;
    }
}
...
async function fetchLatestPosture() {
    try {
        ...
        console.error('[postureHelper] fetchLatestPosture error:', err.message);
        return null;
    }
}
...
function buildHistoryFromResults(results) {
    ...
    return Object.entries(hourMap).map(([hour, group]) => {
        ...
        return {
            ...
            postureScore: score,
            goodPct,
            avgAngle: parseFloat(avgFlex.toFixed(1)),
            count: group.length
        };
    });
}
